...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withUsers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_group
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_group" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_group" returned %v for node %v`, *fk, n.ID)
			}
